...
**include-custom-instructions** *(v1.0.86+)*: By default, custom agents don't automatically pick up repository instruction files. Set `include-custom-instructions: true` in the agent's frontmatter to have it also load `AGENTS.md`, `copilot-instructions.md`, and `CLAUDE.md` from the repository, layering your team's general conventions on top of the agent's own persona:
