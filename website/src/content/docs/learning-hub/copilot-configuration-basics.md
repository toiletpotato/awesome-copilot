...
**Auto routing tier startup defaults** *(v1.0.87+)*: Individual users and organizations can now set a default routing tier (e.g., a specific quality/cost tier) that Auto mode starts from at session launch, instead of always starting from the same built-in default. Organizations can configure this as either a **strict** policy (enforced for every member) or a **user-overridable** default that members can adjust for their own sessions.
...
**`worktreePathTemplate` setting** *(v1.0.87+)*: Customize where `/worktree`, `/move`, `/new`, and `--worktree` create new worktrees using a path template, for example `~/src/worktrees/{repo}/{branch}`. Supported placeholders are `{repoPath}`, `{repo}`, `{branch}`, and `{branchSlug}`. Leaving the setting unset keeps the existing default layout (`<repo>.worktrees/`, with slashes in the branch name flattened to dashes).
