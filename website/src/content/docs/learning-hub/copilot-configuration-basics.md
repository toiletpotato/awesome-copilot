...
| `editorMode` | Set to `vim` to enable Vim modal editing in the composer by default (v1.0.84+) |
...
**"Optimize for" Auto tiers (VS Code 1.139+)**: In VS Code's Copilot Chat and Copilot SDK agent sessions, selecting the **Auto** model now exposes an **Optimize for** control with **Efficiency**, **Balance**, and **Intelligence** options. This lets you bias Auto's server-side routing toward faster/cheaper responses or toward the most capable model available, instead of relying solely on the default balanced routing.
...
> **Session resume reliability (v1.0.85+)**: Resuming an active session without plugin-directory, discovery, or working-directory overrides now preserves marketplace plugins and skills after reload — configuration read or validation failures no longer discard active plugins.

### Context Management Tools for Agents and Subagents

*(v1.0.85+)* Use `/settings` to opt in to additional context management tools that agents and subagents can call directly — for example, tools to summarize or prune earlier turns — giving long-running sessions and delegated subagents more control over their own context budget instead of relying solely on automatic compaction.
