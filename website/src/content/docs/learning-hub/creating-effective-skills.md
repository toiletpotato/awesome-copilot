...
**Q: Can I namespace or exclude skill directories from discovery?**

A: Yes. As of v1.0.88+, the CLI supports namespaced custom skills (grouping related skills under a shared prefix so their names don't collide with other skills) as well as ignored skill directories — folders that are present on disk but explicitly excluded from discovery, useful for keeping drafts or work-in-progress skills out of the loaded set without deleting them.
