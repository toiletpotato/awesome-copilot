...
copilot skill add ./my-skill/ --project # add a skill scoped to this project (v1.0.85+)
...
> **Breaking change (v1.0.85+)**: `copilot skill add --project` replaces the retired `copilot plugins install --skill --scope project` syntax; the `--scope` spelling is no longer accepted.
